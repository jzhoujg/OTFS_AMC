...
% 基本的调制方式的参数设置
N_ti = 16;       % 多普勒索引数；时间间隔
N_sc= 32;       % 时延索引数；一个OFDM符号中含有的子载波数
cp_length = 8; % 循环前缀的长度
...
for mm =hub
    ...
    filename =['./otfs_m/b',num2str(mm-'a'+1),'_otfs_',mode,'/'];
    FILE = [mode,'_']; % 文件命名方式
    ...
    for nn = 1:num
        ...
        sig_rec_2 = reshape(sig_rec,[N_sc+cp_length,N_ti,N_frm]);
        sig_rec_3 = permute(sig_rec_2,[2,1,3]);
        sig_rec=reshape(sig_rec_3,[],1);%由于传输需要
