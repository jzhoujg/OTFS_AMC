% =========================================================================
%  OTFS 信号生成 (OTFS modulation over a multipath-Doppler Rayleigh channel)
% =========================================================================
%  作用: 批量生成 OTFS 接收信号, 每个 .mat 文件保存一个样本的 sig_rec
%        (复基带接收序列, 长度 2560, 供 Python 端 OTFS_OFDM_CNN 分类)。
%
%  调制阶数: bpsk / qpsk / 8psk / 16qam / 64qam / 256qam (变量 hub)。
%  每个样本 = N_frm(=4) 帧, 每帧 (N_sc + cp_length) * N_ti = (64+16)*8 = 640
%            个采样点, 故 sig_rec 长度 = 4 * 640 = 2560。
%  信道    : comm.RayleighChannel (多径 + 最大多普勒频移) + AWGN。
%  SNR     : 从 -10 dB 起, 每生成 num/6 个样本提升 5 dB (覆盖多个 SNR 点)。
%
%  输出: ./otfs_rice/b{1..6}_otfs_{mode}/<mode>_<n>_<SNR>dB.mat
%        字段 sig_rec 即接收信号。二分类任务中, 这些样本统一标记为 "otfs"。
%
%  依赖: MATLAB Communications Toolbox (comm.RayleighChannel / pskmod / qammod /
%        awgn / bi2de)。
%
